...
import { Plus, Send, Users, User, Building2, ChevronLeft, X, Check } from 'lucide-react';
...
import { useMyFriends, FriendProfile } from '../../hooks/useFriends';
...
function DirectChat({ otherId }: { otherId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {conversation.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                {msg.content}
                <div className={`text-[10px] mt-1 ${mine ? 'text-indigo-200' : 'text-slate-500'} text-right`}>
                  {new Date(msg.sentAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                  {mine && (
                    <span className="ml-1">
                      {msg.read ? <span className="text-indigo-300">✓✓</span> : <span className="opacity-60">✓</span>}
                    </span>
                  )}
                </div>
              </div>
      ...
      <form onSubmit={handleSend} className="p-3 border-t border-white/[0.07] flex gap-2">
        <input
          type="text"
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Mensagem..."
          className="flex-1 bg-white/[0.05] border border-white/10 text-white placeholder:text-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
        />
        <button
          type="submit"
          ...
          return (
            <div key={msg.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className="max-w-[70%]">
                {!mine && (
                  <p className="text-[10px] text-indigo-400 mb-1 ml-1">{msg.from_name}</p>
                )}
                <div
...
function CreateGroupModal({ friends, onClose }: { friends: FriendProfile[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="w-full max-w-sm bg-[#0D1025] border border-white/[0.1] rounded-2xl p-6 space-y-4">
        ...
        <form onSubmit={handleCreate} className="space-y-4">
          <input
            ...
            required
            className="w-full bg-white/[0.05] border border-white/10 text-white placeholder:text-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
          />
          <div>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto">
              ...
              {friends.map((f) => (
                ...
                >
                  ...
                  {selected.includes(f.id) && <Check size={14} className="text-indigo-400" />}
                </button>
              ))}
            </div>
          </div>
          <button
            ...
            className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 text-white text-sm font-semibold rounded-xl transition"
          >
...
function ConvoItem({
  active, onClick, name, label, icon: Icon, unread, iconColor = 'text-indigo-400', iconBg = 'bg-indigo-500/20',
}: {
  active: boolean;
  onClick: () => void;
  name: string;
  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition text-left ${
        active ? 'bg-indigo-600/20 border border-indigo-500/30' : 'hover:bg-white/[0.04]'
      }`}
    >
...
export default function AlunoChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] md:h-screen bg-[#080B18] overflow-hidden">
      {showCreateGroup && (
      ...
      <div
        ...
        } w-full md:w-72 flex-col border-r border-white/[0.07] bg-[#0D1025] shrink-0`}
      >
      ...
      <div className={`flex-1 flex flex-col ${!showSidebar ? 'flex' : 'hidden md:flex'}`}>
        {active ? (
          <>
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.07] bg-[#0D1025] shrink-0">
              <button
              ...
              <div
                className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                  active.type === 'personal' ? 'bg-emerald-500/20' :
                  active.type === 'group' ? 'bg-rose-500/20' : 'bg-violet-500/20'
                }`}
              >
                {active.type === 'group' ? (
